<template>
    <div class="card">
        ...
        <!-- {{ rowStore.selectedGinfoRows }} -->
            ...
            <Column selectionMode="multiple" headerStyle="width: 3rem" expander ></Column>
...
onMounted(() => {
    ...
    //     selectedCustomers.value = getCustomers(data);
    // });
...
rowStore.$subscribe((mutation, state) => {
    console.log(mutation); 
    console.log(state);
    ginfo.value = state.RowGinfo;
    // selected.value = ginfo.value;
});
// 載入Glass Table後全選checkbox
watch(() => (ginfo.value),(newGinfo,oldGinfo) =>{
    selected.value = ginfo.value;
});
// 如果GlassTable checkbox項目改變，觸發事件改變rowStore 內selectedGinfoRows內容
watch(() => (selected.value),(newSel,oldSel) => {
    console.log('newSel',newSel);
    if (newSel != []) {
        rowStore.$patch({selectedGinfoRows:newSel})
    }
});
